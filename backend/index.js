...
app.post('/api/projects', authenticateSession, (req, res) => {
  ...
  db.run(
    'INSERT INTO Projects (name, description, createdAt, userId) VALUES (?, ?, ?, ?)',
    [name, description || '', Date.now(), req.user.id],
    function (err) {
      if (err) {
...
app.put('/api/projects/:id', authenticateSession, (req, res) => {
  ...
  db.get('SELECT id FROM Projects WHERE id = ? AND userId = ?', [id, req.user.id], (err, project) => {
    ...
    db.run(
      'UPDATE Projects SET name = ?, description = ?, createdAt = ? WHERE id = ?',
...
app.post('/api/defects', authenticateSession, (req, res) => {
  ...
  db.get('SELECT id FROM Projects WHERE id = ? AND userId = ?', [projectId, req.user.id], (err, project) => {
    ...
    db.run(
      'INSERT INTO Defects (projectId, description, status, createdAt) VALUES (?, ?, ?, ?)',
      [projectId, description, status, Date.now()],
      function (err) {
        if (err) {
...
app.put('/api/defects/:id', authenticateSession, (req, res) => {
  ...
  if (!description || !status) {
    return res.status(400).json({ message: 'Описание и статус дефекта обязательны' });
  }
  if (!['open', 'closed'].includes(status)) {
  ...
  db.get('SELECT d.id, d.projectId FROM Defects d JOIN Projects p ON d.projectId = p.id WHERE d.id = ? AND p.userId = ?', [id, req.user.id], (err, defect) => {
    ...
    db.run(
      'UPDATE Defects SET description = ?, status = ?, createdAt = ? WHERE id = ?',
      [description, status, Date.now(), id],
      (err) => {
        if (err) {
